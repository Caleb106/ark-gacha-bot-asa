...
def iguanadon_open(metadata):
    ...
    while not inventory.is_open():
        attempt += 1
        logger.debug(f"the iguanadon at {metadata.name} could not be accessed retrying {attempt} / {source.gacha_bot.config.iguanadon_attempts}")
        utils.zero()
        utils.set_yaw(metadata.yaw)
        time.sleep(0.2*settings.lag_offset)
        inventory.open()
        if attempt >= source.gacha_bot.config.iguanadon_attempts:
            logger.error(f"the iguanadon at {metadata.name} could not be accesssed after {attempt} attempts")
            break
...
def pickup_seeds():
    ...
    if inventory.is_open():
        ...
        if template.template_await_false(template.check_template,1,"inventory",0.7):
            logger.warning(f"the bag we dropped on the floor for 230 seeds couldnt be fully picked up popcorning now")
            attempts = 0
            while template.check_template("inventory",0.7):
                ...
                if  attempts >= 60 : # 60 * 6  = 360 so whole inv should be popcorned with this value 
                    logger.error("bot got stuck in the popcorning the bag inventory mostlikly broken")
                    break
