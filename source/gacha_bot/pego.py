...
def pego_pickup(metadata):
    ...
    while not inventory.is_open():
        attempt += 1
        logger.debug(f"the pego at {metadata.name} could not be accessed retrying {attempt} / {source.gacha_bot.config.pego_attempts}")
        utils.zero()
        ...
        if attempt >= source.gacha_bot.config.pego_attempts:
            logger.error(f"the pego at {metadata.name} could not be accesssed after {attempt} attempts")
            break
