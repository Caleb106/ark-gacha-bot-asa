...
def open(): 
    ...
    while not is_open():
        attempts += 1
        logger.debug(f"trying to open player inventory {attempts} / {config.inventory_open_attempts}")
        ...
        if attempts >= config.inventory_open_attempts:
            logger.error(f"unable to open up the players inventory")
            break
...
def close():
    ...
    while is_open():
        attempts += 1
        logger.debug(f"trying to close objects inventory {attempts} / {config.inventory_close_attempts}")
        ...
        if attempts >= config.inventory_close_attempts:
            logger.error(f"unable to close the objects inventory after {attempts} attempts")
...
def search_in_inventory(item:str):
    if is_open():
        logger.debug(f"searching in inventory for {item}")
...
def drop_all_inv():  
    if is_open():
        logger.debug(f"dropping all items from our inventory ")
        time.sleep(0.2*settings.lag_offset)
...
def transfer_all_inventory(): 
    if is_open():
        logger.debug(f"transfering all from our inventory into strucutre")
        time.sleep(0.2*settings.lag_offset)
...
def implant_eat():
    ...
    while not template.check_template("death_regions",0.7):
        attempts += 1
        logger.debug(f"trying to eat player implant {attempts} / {config.suicide_attempts}")
        utils.press_key("ShowMyInventory")
        open() 
        close() 
        for x in range(30):
        ...
        if attempts >= config.suicide_attempts:
            logger.error(f"unable to eat player implant")
            break
