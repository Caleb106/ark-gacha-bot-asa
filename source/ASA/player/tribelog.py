...
def open():
    attempts = 0
    logger.debug("trying to open up the tribelog screen")
    while not is_open():
        attempts += 1
        utils.press_key("ShowTribeManager")
        time.sleep(0.1)
        if attempts >= source.ASA.config.tribelog_open_attempts:
            logger.warning(f"tribelogs didnt open in {attempts} attempts")
            break
...
def close():
    ...
    while is_open():
        attempts += 1
        logger.debug(f"trying to close out of the tribelog screen {attempts} / {source.ASA.config.tribelog_close_attempts} ")
