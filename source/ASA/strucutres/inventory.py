...
def open():
    ...
    while not is_open():
        attempts += 1
        logger.debug(f"trying to open strucuture inventory {attempts} / {source.ASA.config.inventory_open_attempts}")
        ...
        if attempts >= source.ASA.config.inventory_open_attempts:
            logger.error(f"unable to open up the objects inventory")
            break
...
def search_in_object(item:str): 
    if is_open():    
        logger.debug(f"searching in structure/dino for {item}")
...
def drop_all_obj():
    if is_open():    
        logger.debug(f"dropping all items from object")
        time.sleep(0.2*settings.lag_offset)
...
def transfer_all_from(): 
    if is_open():
        logger.debug(f"transfering all from object")
        time.sleep(0.2*settings.lag_offset)
