...
def close():
    ...
    while is_open():
        attempts += 1
        logger.debug(f"trying to close the bed {attempts} / {source.ASA.config.teleporter_close_attempts}")
        ...
        if attempts >= source.ASA.config.teleporter_close_attempts:
            logger.error(f"unable to close the bed after {source.ASA.config.teleporter_close_attempts} attempts")
            break
...
def spawn_in(bed_name:str):
    ...
    if is_open():
        ...
        logger.debug(f"char is in the {state}")
        ...
        if not template.template_await_true(template.check_teleporter_orange,3): # waiting for the bed to appear as ready to spawn in
            logger.error(f"the bed char tried spawning on is not in the ready state or cant be found exiting out of bed screen now")
            close()
...
def fast_travel(bed_name:str):
    ...
    if player_state.human.on_bed == False:
        ...
        if player_state.human.on_tp:
            logger.debug(f"char is on a teleporter going to render bed to fast travel")
